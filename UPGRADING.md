...
Cast releases are annotated git tags (`v0.2.0`, `v0.3.0`, `v0.3.1`, …). A hop appears in the table below only when it asks something of you, and ships a migration README at `scripts/migrations/<from>-to-<to>/` when it does. Patch releases normally ask nothing and are simply absent, so an omitted hop is a positive statement: upgrade across it and carry on. Read the table for the hops between your version and the target; if none are listed, there is nothing to do.

## Procedure

1. **Stop the server.**
2. **Check out the target release** (pull the tag, or unpack the release).
3. **Walk the migration READMEs in order** for every *listed* hop between your current version and the target — e.g. upgrading `0.2.0 → 0.3.1` means reading `scripts/migrations/0.2-to-0.3/README.md` and then `scripts/migrations/0.3-to-0.3.1/README.md`. Hops absent from the table have no README and need nothing. Each README covers both halves of its hop:
...
| [`0.3.1 → 0.3.2`](scripts/migrations/0.3.1-to-0.3.2/) | none | **check `routes.json` credentials** — an admin-UI save on 0.3.0/0.3.1 could rotate them |
