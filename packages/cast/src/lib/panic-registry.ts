 ...
 * on threshold N. They miss a gap: a slow runaway loop can burn through
 ...
 *
 * Covers two buttons so far: spawn-rate and time-aware abnormal-exit.
 * Further rate-shaped buttons are deferred.
 */

/** Identifier for which panic button tripped. Each value names a specific
 *  rate-shaped guard; new buttons add new variants here. Drives
...
/** Spawn-rate button — window and threshold. 20 spawns / agent in a
 *  60s window trips. ~10× the worst-case healthy rate of a chatty user
 *  across three channels (~3 spawns/min). A pathological runaway can
 *  peak orders of magnitude above this. */
const SPAWN_RATE_WINDOW_MS = 60_000;
const SPAWN_RATE_MAX = 20;
const SPAWN_RATE_HALT_MS = 10 * 60_000;

/** Abnormal-exit button — time-aware MAX_ABNORMAL_EXITS escalation. Same count
...
export class PanicRegistry {
  ...
  /** Spawn-rate button — record a spawn event for `key` and check the rate
  ...
  /** Abnormal-exit button — time-aware abnormal-exit escalation. Caller is the
